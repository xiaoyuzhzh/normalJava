...
public class RuleMapper {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ScriptEngine engine = getScriptEngine("jsscript/ruleMapper.js");
        Invocable invocable = (Invocable) engine;
        CityRuleDetail ruleMapper = (CityRuleDetail) invocable.invokeFunction("ruleMapper", ruleSnap,Long.MAX_VALUE);
        System.out.println(JSON.toJSONString(ruleMapper,true));
    }

    public static ScriptEngine getScriptEngine(String filePath) throws ScriptException, FileNotFoundException {
        ScriptEngine engine = new ScriptEngineManager().getEngineByName("nashorn");
        String resource = RuleMapper.class.getResource("/").toString();
        String base = resource.replace("file:", "");
//        engine.eval(new FileReader(base+filePath));
//        engine.eval("load('src/script/ruleMapper.js')");
        engine.eval("load('"+base+filePath+"')");
        return engine;
    }
}
